...
**Về Open Charge Map:** dự tính dùng để lấy data trạm thật, nhưng hiện **map đang dùng mock sites**. Khi tích hợp OCM chỉ cần đổi `getSites()` trong `api.ts` — component map không đổi.
...
Types theo doc (`ChargingSession`, `Site`, `FaultRecord`); generator có seed (Boulder/Denver/Palo Alto/Fort Collins/San Jose, ~90 ngày sessions, faults synthetic); `services/api.ts` + `services/mock-data.ts`.

### Phase 2 — Network Overview ✅ (live)
KPI cards (energy/sessions/revenue/users/stations/faults); system status banner; donut connector types; locations by city; map Leaflet + markers + popup; filter toàn trang.
...
- Tích hợp Open Charge Map API thật cho map.
